...
app.use(express.static(path.join(__dirname, 'frontend'))); // تقديم الملفات الثابتة من مجلد frontend
...
// ============= تخزين مؤقت (للتبسيط، يفضل استخدام قاعدة بيانات في الإنتاج) =============
let users = [];
let histories = {};
let nextId = 1;
let nextHistoryId = 1;
...
app.post('/api/generate/text', authMiddleware, async (req, res) => {
    try {
        ...
        if (!apiKey) {
            return res.status(400).json({ error: 'مفتاح API غير متوفر. يرجى إضافة مفتاحك في الإعدادات' });
        }
...
// إرجاع index.html لأي مسار غير معروف (للمساعدة في توجيه React SPA)
app.get('*', (req, res) => {
